...
class ChildWorkbenchController extends Controller
{
     ...
     */
    /**
     * Save the ONE fee for this (root, child) — no more per-service axis.
     *
     * A fee is only written when the child is offered at least one active
     * service — charging for nothing it can sell is a row nothing will ever
     * read, and it is exactly the kind of orphan that made these tables hard
     * to trust.
     */
    public function saveFees(Request $request): RedirectResponse
    {
        $data = $request->validate([
            ...
            'child_id' => ['required', 'integer', 'exists:category_children_master,id'],
            'fee' => ['nullable', 'array'],
            'fee.fee_group_id' => ['nullable', 'integer', 'exists:fee_groups,id'],
        ]);

        $rootId = (int) $data['root_id'];
        $childId = (int) $data['child_id'];

        // `validate()` only returns keys it declared rules for — reading the
        // rest of the row from it silently drops every field but
        // fee_group_id. Read the row raw; fee_group_id's existence was
        // already checked above.
        $input = (array) $request->input('fee', []);
        ...
        if (! $offersAnyService) {
            if (! empty($input['is_active'])) {
                return redirect()
                    ->to(route('admin.child-workbench.index', ['root_id' => $rootId, 'child_id' => $childId], false))
                    ->with('status', __('لا خدمات مفعّلة لهذا الابن — لا يوجد ما يُحصَّل رسمه بعد.'));
            }
            ...
            return redirect()
                ->to(route('admin.child-workbench.index', ['root_id' => $rootId, 'child_id' => $childId], false))
                ->with('status', __('لا خدمات مفعّلة لهذا الابن.'));
        }

        $feeGroupId = ! empty($input['fee_group_id']) ? (int) $input['fee_group_id'] : null;
    ...
    /**
     * What the platform charges on this (root, child) — one rate for every
     * service it offers, not one per service.
     *
     * «بدل ما يكون هناك رسوم بوكينج -منيو -دليفري … رسم موحّد» — المالك،
     * 2026-08-26. Used to live on its own screen —
     * /admin/category-child-service-fees — so «what may this child sell» and
     * «what does it cost to sell it» were two pages that never mentioned each
     * other. Same key, same screen now. The bulk fee screen stays: it answers
     * a different question, «all of these at once».
     */
    private function feePanel(int $rootId, int $childId)
    {
        $fee = CategoryChildServiceFee::query()
            ->where('category_id', $rootId)
            ->where('child_id', $childId)
            ->first();
        ...
        return (object) [
            ...
            'fee_group_id' => $fee?->fee_group_id ? (int) $fee->fee_group_id : null,
