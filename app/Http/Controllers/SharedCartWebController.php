...
class SharedCartWebController extends Controller
{
    ...
    {
        return QrSvg::response(QrSvg::absolute($request, route('cart.shared.join', ['token' => $token], false)));
    }
    ...
    {
        return QrSvg::response(QrSvg::absolute($request, route('table.scan.web', ['token' => $token], false)));
    }
}
