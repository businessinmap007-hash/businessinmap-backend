...
namespace App\Libraries\PayfortPayment;
...
class PayfortIntegration
{
     ...
     */
    public $merchantIdentifier = 'zGQSopvM';

    /**
     * @var string your access code
     */
    public $accessCode = 'iYmlAOr63NLp70DDkSVY';
    ...
    public $priceId;
    public $type;
    public function __construct($priceId = null, $type = null)
    {
    ...
    {
        $price = Pricing::whereId($this->priceId)->first();
        return $price;
    }

    public function getTotalPrice()
    {
        $price = $this->getPricingOptions();
        $total = array_sum([$price->price, $price->ship_price, $price->tax, $price->install_price]);
        return $total;
    }

    private function getOwnerOrder()
    {
        $user = $this->getPricingOptions()->order->user;
        return $user;
    }
    ...
    public function getRedirectionData($paymentMethod)
    {
        ...
        $postData = array(
            'amount' => $this->convertFortAmount($this->getTotalPrice(), $this->currency),
            'currency' => strtoupper($this->currency),
            'merchant_identifier' => $this->merchantIdentifier,
            'access_code' => $this->accessCode,
            'merchant_reference' => $merchantReference,
            'customer_email' => $this->getOwnerOrder()->email,//'test@payfort.com',
//            'customer_name' => trim($order_info['b_firstname'].' '.$order_info['b_lastname']),
            'merchant_extra' => $this->getPricingOptions()->order_id,
            'merchant_extra1' => $this->getPricingOptions()->id,
            'merchant_extra2' => $this->type,
            'command' => $this->command,
            'language' => $this->language,
            'return_url' => route('payment.redirection').'?r=processResponse'//$this->getUrl('ar/payment/redirection?r=processResponse'),
        );
        ...
        $this->log($debugMsg);
        return array('url' => $gatewayUrl, 'params' => $postData, 'order_info' => 'My Order 02');
    ...
    {
        $merchantReference = $this->generateMerchantReference();
        $returnUrl = $this->getUrl('routeeeeeeeeeeeeee.php?r=merchantPageReturn');
        if (isset($_GET['3ds']) && $_GET['3ds'] == 'no') {
            $returnUrl = $this->getUrl('routeaaaaaaaaaaaaa.php?r=merchantPageReturn&3ds=no');
        }
        $iframeParams = array(
            'merchant_identifier' => $this->merchantIdentifier,
            'access_code' => $this->accessCode,
        ...
        if ($paymentMethod == 'installments_merchantpage') {
            ...
            $iframeParams['amount'] = $this->convertFortAmount($this->getTotalPrice(), $this->currency);
        }
    ...
    public function processResponse()
    {
        ...
        if (!$success) {
            $p = $params;
            $p['error_msg'] = $reason;
            $return_url = $this->getUrl(config('app.locale') . '/error/payments?' . http_build_query($p));
        } else {
            //$return_url = $this->getUrl(config('app.locale') . '/success/payments?' . http_build_query($params) . '&orderId=' . $this->priceId);
            if ($params['merchant_extra2'] == "institution") {
                $return_url = route("institution.payment.success") .'?'. http_build_query($params) . '&orderId=' . $this->priceId . '&type=' . $params['merchant_extra2'];
            } else {
                $return_url = $this->getUrl(config('app.locale') . '/success/payments?' . http_build_query($params) . '&orderId=' . $this->priceId . '&type=' . $params['merchant_extra2']);
            }
    ...
    public function merchantPageNotifyFort($fortParams)
    {
        ...
        $postData = array(
            ...
            'language' => $this->language,
            'return_url' => $this->getUrl('route8888888.php?r=processResponse'),
        );
