...
**Exercise library (2026-09-22).** A shared catalogue trainers pick from instead of typing every exercise: `exercise_categories` (sections — chest, back, legs, cardio…) and `exercise_library` (name ar/en, `kind` strength/cardio/flexibility/functional/warmup, `equipment`, suggested sets/reps). `GET /business/training/exercise-library` returns the whole active catalogue (a few hundred rows, filtered client-side) plus the kind/equipment vocabularies. `POST .../exercises` on a plan or a template accepts `library_exercise_id`: `LibraryExercise::withDefaults` fills the name when none is typed and the suggested sets/reps when unset — explicit values win. `plan_exercises`/`template_exercises` keep their OWN name/sets/reps and only remember the entry (`library_exercise_id`, nulled on delete), so editing or deleting the catalogue never changes a running plan. Curated in admin `exercise-library` (CONTENT); `ExerciseLibrarySeeder` is add-only. Plan creation with inline exercises and template→plan apply do not carry the link. Guarded by `ExerciseLibraryTest`.
