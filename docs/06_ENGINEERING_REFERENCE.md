...
**Exercise library (2026-09-22).** A shared catalogue trainers pick from instead of typing every exercise: `exercise_categories` (sections — chest, back, legs, cardio…) and `exercise_library` (name ar/en, `kind` strength/cardio/flexibility/functional/warmup, `equipment`, suggested sets/reps). `GET /business/training/exercise-library` returns the whole active catalogue (a few hundred rows, filtered client-side) plus the kind/equipment vocabularies. `POST .../exercises` on a plan or a template accepts `library_exercise_id`: `LibraryExercise::withDefaults` fills the name when none is typed and the suggested sets/reps when unset — explicit values win. `plan_exercises`/`template_exercises` keep their OWN name/sets/reps and only remember the entry (`library_exercise_id`, nulled on delete), so editing or deleting the catalogue never changes a running plan. Curated in admin `exercise-library` (CONTENT); `ExerciseLibrarySeeder` is add-only. Plan creation with inline exercises and template→plan apply do not carry the link. Guarded by `ExerciseLibraryTest`. **Photos (2026-09-22):** `LibraryExercise` owns an image gallery (`HasOwnedImages` — rows and files die with it). `php artisan exercise-library:import-images` downloads two demonstration frames per exercise from the public-domain Free Exercise DB (https://github.com/yuhonas/free-exercise-db, Unlicense) into `public/files/uploads/exercise-library/` (gitignored — rebuilt per environment, idempotent, `--refresh` to replace, `--dry-run`, `--map=` for a test mapping). The mapping `database/data/exercise_library_image_map.php` (our `name_en` → their folder) is hand-reviewed: 110 of 127 seeded exercises have a genuinely matching picture, the other 17 deliberately have none rather than a wrong one. The library payload carries `images`; a plan exercise created from a library entry exposes `library_images` (trainer and client views) alongside — never mixed into — its own trainer-attached `images`, so the trainer's photo delete route can never touch a library photo. After a deploy run the import command once on the server.

**Private plan photos (2026-09-22).** The photos a trainer attaches to a plan's exercises and meals are private to that trainer and the plan's trainee: they are stored OUTSIDE the web root (`storage/app/plan-photos/<random>.<ext>`, `ImageUploadService::storePrivate`), never under `public/files/uploads`. A plan payload hands out `PlanPhotoUrl` links — signed, time-limited (snapped to 6-hour boundaries so the URL is stable for the app's image cache; a link lives 6–12 h) — served by `GET /api/v2/plan-photos/{image}` (`signed` middleware, no login, so a plain image widget can load it). The link is the credential: it is only ever placed in a plan payload the two parties may already read, and a link can be shared by whoever holds it until it expires. The route serves only `PlanExercise`/`PlanMeal` images stored under the private prefix. Deleting a photo, or the exercise/meal/plan owning it, deletes the file (`ImageUploadService::delete` understands both locations). The exercise library's public-domain photos are unaffected and stay public. Guarded by `TrainingPlanImagesTest`.

**Set logging + monthly summary (2026-09-23).** A confirmed round now records the reps and weight actually done (`plan_exercise_rounds.reps/weight`, numbers only — still no free text or images from the trainee), next to the trainer's prescribed `target_weight` on plan/template exercises. `plan_session_completions` is written once per plan per day when every exercise OWED that day (scheduled for that weekday or 'any day', with sets > 0) has all its sets confirmed; only the call that creates the row sends the trainer `training_session_completed` (action `open_training_plan_manage`), so a correction never re-notifies. Volume = reps x weight over the sets that recorded BOTH — a set confirmed without numbers counts as a set but adds no volume. `GET .../monthly-summary?month=YYYY-MM` (client and trainer sides) is computed from the rounds themselves, so nothing has to be saved into it; `GET business/training-plans/{plan}/log?date=` is the trainer's day view against the prescription; the trainee can correct a set via `PUT .../rounds/{round}`. Landmine: `TrainingPlan::progressLogs()` is newest-first, so any 'first reading of the month' must `reorder()`. Guarded by `TrainingPerformanceTest`.

**Trainer photo library (2026-09-23).** `trainer_photos` is a trainer's private library (owned by the acting business, so a `training` delegate shares the gym's; max 300), stored in the same private storage as plan photos and served by the signed `GET /trainer-photos/{photo}` link. Attaching a library photo to a client's exercise or meal (`POST .../images/from-library`, `photo_ids[]`) COPIES the file (`ImageUploadService::copyPrivate`) into that plan — one photo can serve any number of clients while every plan stays self-contained, so deleting the library photo or one client's exercise never breaks another client's plan. A photo id outside the caller's own library is a 404, never a hint that it exists. An ordinary upload can also pass `save_to_library` to keep a copy. Guarded by `TrainerPhotoLibraryTest`.

**Programme weeks + progressive overload (2026-09-24).** A plan is a programme: the exercises are defined ONCE (a week's template — Push/Pull/Legs on their days, `day_label`), `duration_weeks` says how long it runs, and the weights climb by a rule on each exercise: base weight per set (`set_weights`, e.g. 20-25-30, or the flat `target_weight`) + `floor((week-1) / progress_every_weeks) x progress_increment_kg`. Nothing is stored per week: `App\Support\ExerciseProgression` computes any week's per-set targets, so editing the rule re-plans every later week at once. Week 1 is the plan's `starts_on` (else its creation date). One call builds it all: `POST business/training-plans` takes `duration_weeks`, a plan-level default `progression{every_weeks,increment_kg}` (applied to exercises that have weights and no rule of their own) and each exercise's `set_weights`/`progress_*`/`day_label`; `PUT .../program` re-plans length + climb for the whole plan (or chosen `exercise_ids`); `PATCH .../exercises/{id}` edits one. Templates carry the same fields and `apply` takes `duration_weeks`. Trainer payloads carry `current_targets` (this week per set) and the full `schedule`; the trainee's carry `current_targets` and `next_targets` (only when next week differs), plus `week_number`/`total_weeks`. `TrainingProgramRules` is the single validator/normaliser for the fields. Weight strings are read by `ExerciseProgression::parseWeights` — a comma or dash SEPARATES weights ("20,25,30" is three), only a dot is a decimal. Guarded by `TrainingProgramTest`.

**Trainer client picker (2026-09-24).** `GET /business/training/clients/lookup?q=` finds the client a plan is for by an EXACT phone or e-mail (client accounts only, never the trainer himself), throttled 30/min — deliberately not a search, so it can never be a people-finder (same rule as the web panel's picker). It unblocks applying a template from the app (`POST .../training-templates/{id}/apply` with the chosen weeks and start date): the new plan is created pending and sent to the client to accept. Guarded by `TrainerClientLookupTest`. Per the owner's 2026-09-24 rule, this feature has no admin-panel page — it lives in the app.

**Lapse alert (2026-09-25).** `training:alert-lapses` (daily 10:00; `--date=` to evaluate as of a date) tells a trainer when a client has missed their last 2 SCHEDULED training days (`TrainingPerformanceService::lapseStreak/alertLapses`, notification `training_lapse`, opens the plan). A day is scheduled only when a weekday-specific exercise (`day_of_week` not null, sets > 0) is owed on it — an «any day» exercise is owed every day and so never defines a schedule, or anyone training three times a week would look lapsed. A day counts as missed when not one set was logged on it; the run stops at the first day with activity; days before the later of the start date and the plan's creation day are never owed. Once per lapse: `training_plans.lapse_alerted_on` holds the newest missed day an alert covered, and nothing more is sent until a set has been logged after it. Only ACTIVE plans are watched. No screen: the trainer receives it as a notification, per the no-admin-pages rule. Guarded by `TrainingLapseAlertTest`.
