...
<div class="a2-page">
    <div class="a2-page-head">
        <div>
            <h1 class="a2-page-title">{{ __('الأقسام') }}</h1>
            <div class="a2-page-subtitle">
                {{ __('إدارة الأقسام الرئيسية، الفروع الموحدة، وربط الخدمات والرسوم') }}
            </div>
        </div>
    ...
    <div class="a2-stat-grid a2-mb-16">
        <div class="a2-stat-card">
            <div class="a2-stat-label">{{ __('الأقسام الرئيسية') }}</div>
            <div class="a2-stat-value">{{ $rootsSafe->count() }}</div>
            <div class="a2-stat-note">Root Categories</div>
        </div>

        <div class="a2-stat-card">
            <div class="a2-stat-label">{{ __('القسم المحدد') }}</div>
    ...
    <div class="a2-card a2-mb-16">
        <form method="GET" action="{{ route('admin.categories.index') }}" class="a2-filterbar">
            ...
            <select class="a2-select a2-filter-md" name="root_id">
                <option value="0" @selected($rootIdInt === 0)>{{ __('كل الأقسام الرئيسية') }}</option>
    ...
    @if($rootIdInt > 0 && !empty($root))
        <div class="a2-card a2-card--section a2-mb-16">
            <div class="a2-card-head">
                <div>
                    <div class="a2-section-title a2-mb-0">{{ __('القسم الرئيسي المختار') }}</div>
        ...
        <form method="POST"
            ...
            <div class="a2-card-head">
                <div>
                    ...
                    <div class="a2-section-subtitle">
                        {{ __('تطبيق الخدمات والرسوم على الفروع المحددة دفعة واحدة') }}
                    </div>
                </div>
            </div>
            ...
            @if($childrenCount > 0)
                <div class="a2-form-grid">
                    <div>
                        <label class="a2-label">{{ __('الأقسام الفرعية') }}</label>
                        ...
                        <div class="a2-muted a2-mt-8">{{ __('يمكن اختيار أكثر من فرع.') }}</div>
                    </div>

                    <div>
                        <label class="a2-label">{{ __('الخدمات') }}</label>
                        ...
                        <div class="a2-muted a2-mt-8">{{ __('الخدمات المختارة سيتم ربطها بالفروع.') }}</div>
                    </div>
                </div>
                ...
                <div class="a2-form-grid">
                    <div>
                        <label class="a2-label">{{ __('طريقة التطبيق') }}</label>
                        <select name="mode" class="a2-select" required>
                            <option value="append">{{ __('إضافة / تحديث') }}</option>
                            <option value="replace">{{ __('استبدال كامل') }}</option>
                            <option value="remove">{{ __('حذف الربط') }}</option>
                        </select>
                    </div>
                ...
                <div class="a2-form-grid a2-mt-12">
                    <div class="a2-card a2-card--soft" style="padding:14px;">
                        <label class="a2-label">{{ __('رسوم البزنس') }}</label>
                    ...
                    <div class="a2-card a2-card--soft" style="padding:14px;">
                        <label class="a2-label">{{ __('رسوم العميل') }}</label>
                        ...
                        <input type="number"
                               ...
                               class="a2-input"
                               placeholder="{{ __('مثال: 3') }}">
                    </div>
                </div>

                <div class="a2-mt-12">
                    <label class="a2-label">{{ __('ملاحظات الرسوم') }}</label>
                    <input type="text"
                           name="fee_notes"
                           class="a2-input"
            ...
            @else
                <div class="a2-alert a2-alert-warning">
                    {{ __('لا توجد فروع لهذا القسم الرئيسي حتى الآن.') }}
                </div>
            @endif
        </form>
    @endif
            ...
            <table class="a2-table">
                <thead>
                    <tr>
                        ...
                        <th>
                            <a href="{{ $sortUrl('name_en') }}">{{ __('الاسم الإنجليزي') }}{!! $arrow('name_en') !!}</a>
                        </th>
                        <th>
                            <a href="{{ $sortUrl('reorder') }}">{{ __('الترتيب') }}{!! $arrow('reorder') !!}</a>
                        </th>
                        <th>{{ __('الحالة') }}</th>
                        <th style="width:260px;">{{ __('إجراءات') }}</th>
                    </tr>
                </thead>
                ...
                <tbody>
                    @if($rootIdInt > 0)
                        ...
                        @empty
                            <tr>
                                <td colspan="6">
                                    <div class="a2-empty">{{ __('لا توجد أقسام فرعية.') }}</div>
                                </td>
                            </tr>
                        @endforelse
                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="6">
                                    <div class="a2-empty">{{ __('لا توجد أقسام رئيسية.') }}</div>
                                </td>
                            </tr>
                        @endforelse
                    @endif
