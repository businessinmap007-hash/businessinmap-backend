...
<div class="a2-page">
    <div class="a2-page-head">
        <div>
            <h1 class="a2-page-title">{{ __('محرك الضمان') }}</h1>
            <div class="a2-page-subtitle">{{ __('متابعة ضمانات العملاء والأعمال، مستويات التغطية، الرصيد المجمد، Grace وExpiration.') }}</div>
        </div>
        <div class="a2-page-actions">
            <a href="{{ route('admin.wallet-ops.recharge.form') }}" class="a2-btn a2-btn-primary">{{ __('شحن محفظة / ترقية ضمان') }}</a>
        </div>
    </div>

    <div class="a2-stat-grid a2-mb-16">
        <div class="a2-stat-card">
            <div class="a2-stat-label">{{ __('إجمالي الضمانات') }}</div>
            <div class="a2-stat-value">{{ number_format($totals['count'] ?? 0) }}</div>
            <div class="a2-stat-note">{{ __('حسب الفلاتر الحالية') }}</div>
        </div>
        <div class="a2-stat-card">
            <div class="a2-stat-label">{{ __('نشط / Pending') }}</div>
            <div class="a2-stat-value">{{ number_format($totals['active'] ?? 0) }} / {{ number_format($totals['pending'] ?? 0) }}</div>
            <div class="a2-stat-note">{{ __('تغطية كاملة أو مؤقتة') }}</div>
        </div>
        <div class="a2-stat-card">
            ...
            <div class="a2-stat-note">{{ __('تحتاج مراجعة أو انتظار Grace') }}</div>
        </div>
        <div class="a2-stat-card">
            <div class="a2-stat-label">{{ __('الرصيد المجمد / التغطية') }}</div>
    ...
    <div class="a2-card a2-card--tight">
        <form method="GET" action="{{ route('admin.guarantees.index') }}" class="a2-filterbar">
            ...
            <select class="a2-select a2-filter-md" name="level_id">
                <option value="0">{{ __('كل المستويات') }}</option>
            ...
            <select class="a2-select a2-filter-md" name="expires">
                ...
                <option value="has_expiration" {{ $expires === 'has_expiration' ? 'selected' : '' }}>{{ __('له تاريخ انتهاء') }}</option>
                <option value="expired" {{ $expires === 'expired' ? 'selected' : '' }}>{{ __('منتهي') }}</option>
                <option value="missing" {{ $expires === 'missing' ? 'selected' : '' }}>{{ __('بدون تاريخ') }}</option>
            </select>
            ...
            <div class="a2-filter-actions">
                ...
                <a class="a2-btn a2-btn-ghost" href="{{ route('admin.guarantees.index') }}">{{ __('إعادة ضبط') }}</a>
            </div>
        </form>
    </div>
        ...
        <div class="a2-table-wrap">
            <table class="a2-table">
                ...
                <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="a2-actions">
                                    <a href="{{ route('admin.guarantees.show', $guarantee->id) }}" class="a2-btn a2-btn-sm a2-btn-ghost">{{ __('عرض') }}</a>
                                    @if($user)
                    ...
                    @empty
                        <tr>
                            <td colspan="14" class="a2-empty-cell">{{ __('لا توجد ضمانات مطابقة للفلاتر.') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
