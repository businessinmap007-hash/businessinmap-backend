...
<div class="a2-page a2-page-narrow">
    <div class="a2-page-head">
        <div>
            <h1 class="a2-page-title">{{ __('تعديل عنصر منيو') }}</h1>
            <div class="a2-page-subtitle">{{ $row->name_ar ?: ('#' . $row->id) }}</div>
        </div>

        <div class="a2-page-actions">
            <a class="a2-btn a2-btn-ghost" href="{{ route('admin.menu-items.index') }}">{{ __('رجوع') }}</a>
        </div>
    </div>
    ...
    <div class="a2-card" style="margin-top:16px;">
        <h2 class="a2-section-title">{{ __('Variants (مقاسات / أنواع)') }}</h2>
                ...
                @forelse($row->variants as $variant)
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <button type="submit" form="{{ $variantFormId }}" class="a2-btn a2-btn-ghost a2-btn-sm">{{ __('حفظ') }}</button>
                            ...
                            <form method="POST" action="{{ route('admin.menu-items.variants.destroy', [$row, $variant]) }}" onsubmit="return confirm('حذف الـ variant؟');" style="display:inline;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="a2-btn a2-btn-danger a2-btn-sm">{{ __('حذف') }}</button>
                            </form>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="7" class="a2-empty-cell">{{ __('لا توجد variants') }}</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
        ...
        <form method="POST" action="{{ route('admin.menu-items.variants.store', $row) }}" class="a2-form-grid" style="margin-top:12px;">
            @csrf
            <input class="a2-input" type="text" name="type" placeholder="{{ __('النوع (size/color..)') }}" required>
            <input class="a2-input" type="text" name="name_ar" placeholder="{{ __('الاسم بالعربي') }}" required>
            <input class="a2-input" type="number" step="0.01" name="price" placeholder="{{ __('سعر مباشر') }}">
            <input class="a2-input" type="number" step="0.01" name="price_delta" placeholder="{{ __('فرق عن السعر الأساسي') }}">
            <label class="a2-checkbox-label"><input type="checkbox" name="is_default" value="1"> {{ __('افتراضي') }}</label>
            <label class="a2-checkbox-label"><input type="checkbox" name="is_active" value="1" checked> {{ __('مفعّل') }}</label>
            <button type="submit" class="a2-btn a2-btn-primary">{{ __('+ إضافة variant') }}</button>
        </form>
    </div>

    <div class="a2-card" style="margin-top:16px;">
        <h2 class="a2-section-title">{{ __('Extras (إضافات)') }}</h2>
        ...
        <div class="a2-table-wrap">
            <table class="a2-table">
                ...
                @forelse($row->extras as $extra)
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <button type="submit" form="{{ $extraFormId }}" class="a2-btn a2-btn-ghost a2-btn-sm">{{ __('حفظ') }}</button>
                ...
                @empty
                    <tr><td colspan="6" class="a2-empty-cell">{{ __('لا توجد إضافات') }}</td></tr>
                @endforelse
                </tbody>
            </table>
        </div>
        ...
        <form method="POST" action="{{ route('admin.menu-items.extras.store', $row) }}" class="a2-form-grid" style="margin-top:12px;">
            @csrf
            <input class="a2-input" type="text" name="group_key" placeholder="{{ __('المجموعة (Sauces..)') }}">
            <input class="a2-input" type="text" name="name_ar" placeholder="{{ __('الاسم بالعربي') }}" required>
            <input class="a2-input" type="number" step="0.01" name="price" placeholder="{{ __('السعر') }}" required>
            <input class="a2-input" type="number" name="max_qty" placeholder="{{ __('أقصى كمية') }}">
            <label class="a2-checkbox-label"><input type="checkbox" name="is_active" value="1" checked> {{ __('مفعّل') }}</label>
            <button type="submit" class="a2-btn a2-btn-primary">{{ __('+ إضافة extra') }}</button>
        </form>
    </div>
</div>
@endsection
