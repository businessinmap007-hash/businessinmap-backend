...
<div class="a2-page">
  <div class="a2-card" style="max-width:820px">
    <div class="a2-header">
      <div>
        <h2 class="a2-title">إعدادات الإشعارات — Firebase (FCM)</h2>
        <div class="a2-hint">الصق ملف الحساب الخدمي (Service Account JSON) من إعدادات مشروع Firebase. يُحفَظ مشفّرًا ويُستخدم في الإرسال التالي دون تعديل الكود أو إعادة النشر.</div>
      </div>
    </div>
    ...
    <div class="a2-field" style="margin-bottom:14px">
      ...
      @else
        <div class="a2-hint">لم تُضبَط بيانات اعتماد بعد — الإشعارات لن تصل للأجهزة حتى تُحفَظ.</div>
      @endif
    </div>
    ...
    <form method="post" action="{{ route('admin.push-settings.update') }}">
      ...
      <div class="a2-actionsbar" style="margin-top:8px">
        <button class="a2-btn a2-btn-primary" type="submit">حفظ بيانات الاعتماد</button>
      </div>
    </form>

    <form method="post" action="{{ route('admin.push-settings.test') }}" style="margin-top:12px">
      @csrf
      <button class="a2-btn a2-btn-ghost" type="submit" @disabled(! $firebase['is_set'])>اختبار الاتصال بـ Firebase</button>
      <span class="a2-hint">يتحقّق من صحّة الملف بمحاولة الحصول على رمز وصول من Google.</span>
    </form>

    <div class="a2-alert a2-alert-muted" style="margin-top:20px">
      <strong>ملاحظات:</strong>
      <ul style="margin:8px 18px 0;line-height:1.9">
        <li>الملف يحوي مفتاحًا خاصًّا — يُخزَّن مشفّرًا ولا يُعرَض مرّة أخرى بعد الحفظ.</li>
        <li>يستقبل التطبيق رموز الأجهزة عبر <code dir="ltr">POST /api/v2/push-tokens</code>؛ الإرسال يتمّ لأجهزة المستخدم النشطة فقط.</li>
        <li>يبقى <code dir="ltr">FCM_SERVICE_ACCOUNT_JSON</code> في ملف env يعمل كبديل احتياطي إن لم تُضبَط هنا.</li>
      </ul>
    </div>
  </div>
</div>
