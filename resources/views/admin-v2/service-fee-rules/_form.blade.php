...
<div class="a2-card a2-card--section">
    <div class="a2-card-head">
        <div>
            <div class="a2-card-title">{{ __('القاعدة') }}</div>
            <div class="a2-card-sub">{{ __('اتركِ النطاق فارغًا ليشمل الجميع. الأولوية الأقل تُطبَّق أولًا، والقواعد المتطابقة تتراكم.') }}</div>
        </div>
    </div>

    <div class="a2-form-grid">
        <div class="a2-form-group" style="grid-column:1/-1;">
            <label class="a2-label" for="name">{{ __('اسم القاعدة') }} <span class="a2-danger">*</span></label>
            <input class="a2-input" id="name" name="name" value="{{ old('name', $rule->name ?? '') }}" placeholder="{{ __('زيادة وقت الذروة — القاهرة') }}" required>
        </div>

        <div class="a2-form-group">
            <label class="a2-label" for="platform_service_id">{{ __('الخدمة') }}</label>
        ...
        <div class="a2-form-group">
            <label class="a2-label" for="payer">{{ __('الطرف الذي تُعدَّل رسومه') }} <span class="a2-danger">*</span></label>
        ...
        <div class="a2-form-group">
            <label class="a2-label" for="priority">{{ __('الأولوية') }}</label>
            <input class="a2-input" id="priority" name="priority" type="number" min="0" value="{{ old('priority', $rule->priority ?? 100) }}">
            <div class="a2-hint a2-mt-8">{{ __('الأقل يُطبَّق أولًا.') }}</div>
        </div>

        <div class="a2-form-group">
            <label class="a2-label">{{ __('إيقاف التسلسل') }}</label>
            <label class="a2-check" style="margin-top:10px;">
                ...
                <span>{{ __('لا تُطبَّق أي قاعدة بعد هذه') }}</span>
            </label>
        </div>
...
<div class="a2-card a2-card--section">
    <div class="a2-card-head">
        <div>
            <div class="a2-card-title">{{ __('التأثير على الرسوم') }}</div>
            <div class="a2-card-sub">{{ __('النسبة والمبلغ يقبلان قيمة سالبة للخصم. الحدود تُطبَّق بعد التأثير، والرسوم لا تنزل تحت الصفر أبدًا.') }}</div>
        </div>
    </div>

    <div class="a2-form-grid">
        <div class="a2-form-group">
            <label class="a2-label" for="effect">{{ __('التأثير') }} <span class="a2-danger">*</span></label>
        ...
        <div class="a2-form-group js-sfr-effect-value">
            <label class="a2-label" for="effect_value">{{ __('القيمة') }}</label>
            <input class="a2-input" id="effect_value" name="effect_value" type="number" step="0.01" value="{{ old('effect_value', $rule->effect_value ?? '') }}" placeholder="{{ __('50 = +50%، -10 = خصم 10') }}">
        </div>

        <div class="a2-form-group">
            <label class="a2-label" for="min_fee">{{ __('حد أدنى للرسوم') }}</label>
            <input class="a2-input" id="min_fee" name="min_fee" type="number" step="0.01" min="0" value="{{ old('min_fee', $rule->min_fee ?? '') }}">
        </div>

        <div class="a2-form-group">
            <label class="a2-label" for="max_fee">{{ __('حد أقصى للرسوم') }}</label>
            <input class="a2-input" id="max_fee" name="max_fee" type="number" step="0.01" min="0" value="{{ old('max_fee', $rule->max_fee ?? '') }}">
        </div>

        <div class="a2-form-group">
            <label class="a2-label" for="starts_at">{{ __('تبدأ في') }}</label>
        ...
        <div class="a2-form-group">
            <label class="a2-label" for="ends_at">{{ __('تنتهي في') }}</label>
...
<div class="a2-card a2-card--section">
    <div class="a2-card-head">
        <div>
            <div class="a2-card-title">{{ __('الشروط') }}</div>
            <div class="a2-card-sub">
                {{ __('كل الشروط المملوءة يجب أن تتحقق معًا. الحقل الفارغ يعني «لا يهم» — واترك الكل فارغًا لتطبيق القاعدة على النطاق كله.') }}
            </div>
        </div>
    </div>
    ...
    <div class="a2-form-grid">
        <div class="a2-form-group">
            <label class="a2-label" for="c_min_base_amount">{{ __('أقل قيمة للعملية') }}</label>
        ...
        <div class="a2-form-group">
            <label class="a2-label" for="c_max_base_amount">{{ __('أعلى قيمة للعملية') }}</label>
        ...
        <div class="a2-form-group">
            <label class="a2-label" for="c_min_success_operations">{{ __('أقل عدد عمليات ناجحة') }}</label>
        ...
        <div class="a2-form-group">
            <label class="a2-label" for="c_max_success_operations">{{ __('أعلى عدد عمليات ناجحة') }}</label>
        ...
        <div class="a2-form-group">
            <label class="a2-label" for="c_time_from">{{ __('من الساعة') }}</label>
            <input class="a2-input" id="c_time_from" name="c_time_from" type="time" value="{{ old('c_time_from', $c['time_from'] ?? '') }}">
        </div>

        <div class="a2-form-group">
            <label class="a2-label" for="c_time_to">{{ __('إلى الساعة') }}</label>
            <input class="a2-input" id="c_time_to" name="c_time_to" type="time" value="{{ old('c_time_to', $c['time_to'] ?? '') }}">
            <div class="a2-hint a2-mt-8">{{ __('نافذة تعبر منتصف الليل مسموحة (22:00 → 02:00).') }}</div>
        </div>

        <div class="a2-form-group">
            <label class="a2-label" for="c_subscribed">{{ __('اشتراك البزنس') }}</label>
            <select class="a2-select" id="c_subscribed" name="c_subscribed">
                <option value="" @selected($currentSubscribed === '')>{{ __('لا يهم') }}</option>
                <option value="1" @selected($currentSubscribed === '1')>{{ __('مشترك فقط') }}</option>
                <option value="0" @selected($currentSubscribed === '0')>{{ __('غير مشترك فقط') }}</option>
            </select>
        </div>

        <div class="a2-form-group">
            <label class="a2-label" for="c_days_of_week">{{ __('أيام الأسبوع') }}</label>
        ...
        <div class="a2-form-group">
            <label class="a2-label" for="c_governorate_ids">{{ __('المحافظات') }}</label>
        ...
        <div class="a2-form-group">
            <label class="a2-label" for="c_service_keys">{{ __('أنواع الخدمة') }}</label>
