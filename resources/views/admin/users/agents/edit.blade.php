...
@endsection
@section('content')
    <form id="storeCampaign" method="POST" action="{{ route('update.agent', $agent->id) }}"
                ...
                <div class="card-box">
                            ...
                            <div class="col-xs-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name_{{$locale}}"
                                           value="{{ getTextForAnotherLang($agent, 'name', $locale) }}"
                            ...
                            <div class="col-xs-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="requirements_{{$locale}}"
                                           value="{{ getTextForAnotherLang($agent, 'requirements', $locale) }}"
                                           class="form-control" required
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control" name="agentType" required
                                    ...
                                    @foreach($agentstypes as $agentstype)
                                        <option value="{{ $agentstype->id }}"
                                                @if($agent->service_id == $agentstype->id) selected @endif>{{ getTextForAnotherLang($agentstype, 'name', app()->getLocale()) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-xs-6">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="" required id="selectCountry"
                                    ...
                                    @if($agent->city->country)
                                        @foreach($countries as $country)
                                            <option value="{{ $country->id }}"
                                                    @if($agent->city->country->id == $country->id) selected @endif>{{ getTextForAnotherLang($country, 'name', app()->getLocale()) }}</option>
                                        @endforeach
                        ...
                        <div class="col-xs-6">
                            <div class="form-group{{ $errors->has('city_id') ? ' has-error' : '' }}">
                                ...
                                <select class="form-control" required name="city_id" id="selectCity"
                                    ...
                                    @if($cuntry = $agent->city->country)
                                        @foreach($cuntry->cities as $city)
                                            <option value="{{ $city->id }}"
                                                    @if($agent->city_id == $city->id) selected @endif>{{ getTextForAnotherLang($city, 'name', app()->getLocale()) }}</option>
                                        @endforeach
                                    @endif
                        ...
                        <div class="col-xs-6">
                            <div class="form-group">
                                ...
                                <textarea type="text" name="address_{{ $locale }}" class="form-control"
                                          ...
                                          data-parsley-minlength="3"
                                          data-parsley-minlength-message="اقل عدد حروف مسموح به هو 3 حروف"
                                >{{ getTextForAnotherLang($agent, 'address', $locale) }}</textarea>
                                <p class="help-block" id="error_userName"></p>
                        ...
                        <div class="col-xs-6">
                            <div class="form-group">
                                ...
                                <textarea type="text" name="description_{{ $locale }}" class="form-control"
                                          ...
                                          data-parsley-minlength="3"
                                          data-parsley-minlength-message="اقل عدد حروف مسموح به هو 3 حروف"
                                >{{ getTextForAnotherLang($agent, 'description', $locale) }}</textarea>
                                <p class="help-block" id="error_userName"></p>
                    ...
                    <div class="col-xs-6">
                        <div class="form-group{{ $errors->has('root_type') ? ' has-error' : '' }}">
                            ...
                            <select class="form-control" required name="root_type" id="selectCity"
                                ...
                                <option value="airport" @if($agent->seats_no == 'airport') selected @endif>@lang('trans.airport')</option>
                                <option value="permit" @if($agent->seats_no == 'permit') selected @endif>@lang('trans.permit')</option>
                                <option value="hotels" @if($agent->seats_no == 'hotels') selected @endif>@lang('trans.hotels')</option>
                                <option value="transport" @if($agent->seats_no == "transport") selected @endif>@lang('trans.transport')</option>
                    ...
                    <div class="col-xs-6">
                        ...
                        <div class="form-group" id="weeksActivity"
                             @if($agent->permit_no == 1) style="display: none;" @endif>
                            <label for="userName">عدد الاسابيع</label>



                            <input class="form-control" name="weeks_no" value="{{ $agent->mina_locations }}"/>

                            {{--<select class="form-control" name="weeks_no" required--}}
                                    {{--data-parsley-required-message="تحدد عدد الاسابيع مطلوب"--}}
                                    {{--data-parsley-trigger="change">--}}
                                {{--<option value="1" @if((int)$agent->mina_locations == 1) selected @endif> اسبوع</option>--}}
                                {{--<option value="2" @if((int)$agent->mina_locations == 2) selected @endif> اسبوعين</option>--}}
                                {{--<option value="3" @if((int)$agent->mina_locations == 3) selected @endif> 3 اسابيع</option>--}}

                            {{--</select>--}}
                        </div>


                        <div class="form-group" id="dataRangeActivity"
                             @if($agent->permit_no == 0) style="display: none;" @endif>
                            <label class="control-label"> التاريخ من / الي</label>

                            <?php
                                $dates = $agent->mina_locations;
                                $data = explode(',', $dates);

                            ?>
                            <div class="input-daterange input-group" id="date-range">
                                <input type="text" class="form-control" name="dateFrom" value="{{ $agent->permit_no == 1 ? $data[0] : "" }}"/>
                                <span class="input-group-addon bg-primary b-0 text-white">إلي</span>
                                <input type="text" class="form-control" name="dateTo" value="{{ $agent->permit_no == 1 ? $data[1] : "" }}"/>
                            </div>
