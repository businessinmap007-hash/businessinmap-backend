...
@section('title', __('حجز #:id', ['id' => $booking->id]))
...
@php
    ...
    $modeLabels = ['free' => __('مجانية'), 'reservation_fee' => __('رسوم حجز'), 'minimum_charge' => __('حد أدنى'), 'standard' => __('سعر عادي')];
@endphp

<div class="a2-page-head">
    <div>
        <h1 class="a2-page-title">{{ __('حجز #:id', ['id' => $booking->id]) }}</h1>
        <div class="a2-page-subtitle">{{ $serviceName }} — {{ $unitCode }} <span dir="ltr">({{ $unitType }})</span></div>
    </div>
    <div class="a2-page-actions">
        <a href="{{ route('business.bookings.index') }}" class="a2-btn a2-btn-ghost">{{ __('رجوع') }}</a>
    </div>
</div>
...
<div class="a2-form-grid">
        ...
        <div class="a2-card a2-card--section">
            ...
            <div class="a2-table-wrap">
                <table class="a2-table">
                    <thead>
                        <tr><th>{{ __('الصنف') }}</th><th>{{ __('السعر') }}</th><th>{{ __('الكمية') }}</th><th>{{ __('الإجمالي') }}</th><th></th></tr>
                    </thead>
                    <tbody>
                        @forelse($lines as $line)
                            <tr>
                                ...
                                <td class="a2-text-right">
                                    <form method="POST" action="{{ route('business.bookings.food.remove', $booking->id) }}" onsubmit="return confirm('{{ __('حذف الصنف؟') }}');">
                                        @csrf
                                        @method('DELETE')
                        ...
                        @empty
                            <tr><td colspan="5" class="a2-empty">{{ __('لا يوجد أكل مضاف لهذا الحجز.') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
    ...
    <div>
        <div class="a2-card a2-card--section">
            <div class="a2-card-head"><div><div class="a2-card-title">{{ __('الفاتورة الموحّدة') }}</div></div></div>

            <table class="a2-table">
                <tbody>
                    <tr>
                        <td>{{ __('احتساب الوحدة') }}</td>
                        <td class="a2-text-right">
                            <span class="a2-pill a2-pill-sub">{{ $modeLabels[$invoice['charge_mode']] ?? $invoice['charge_mode'] }}</span>
                        </td>
                    </tr>
                    <tr><td>{{ __('رسوم الوحدة / الطاولة') }}</td><td class="a2-text-right a2-fw-900">{{ number_format((float) $invoice['table_charge'], 2) }} {{ $cur }}</td></tr>
                    <tr><td>{{ __('إجمالي الأكل') }}</td><td class="a2-text-right a2-fw-900">{{ number_format((float) $invoice['food_total'], 2) }} {{ $cur }}</td></tr>
                    <tr><td class="a2-fw-900">{{ __('الإجمالي') }}</td><td class="a2-text-right a2-fw-900">{{ number_format((float) $invoice['total'], 2) }} {{ $cur }}</td></tr>
                    <tr><td>{{ __('التأمين (ضمان)') }}</td><td class="a2-text-right">{{ number_format((float) $invoice['deposit_amount'], 2) }} {{ $cur }}</td></tr>
                </tbody>
            </table>
