...
<div class="a2-card a2-card--section" style="margin-top:20px;">
    <div class="a2-card-head">
        <div>
            <div class="a2-card-title">{{ __('صور الصنف') }}</div>
            <div class="a2-card-sub">{{ __('حتى ١٠ صور. تُحذف نهائيًا مع الصنف.') }}</div>
        </div>
    </div>
    ...
    <div style="display:flex;flex-wrap:wrap;gap:12px;margin-bottom:14px;">
        @forelse($row->images as $img)
            <div style="position:relative;width:120px;">
                ...
                <form method="POST" action="{{ route('business.menu.images.destroy', [$row->id, $img->id]) }}" onsubmit="return confirm('{{ __('حذف هذه الصورة نهائيًا؟') }}')" style="margin-top:6px;">
                    @csrf @method('DELETE')
    ...
    <form method="POST" action="{{ route('business.menu.images.store', $row->id) }}" enctype="multipart/form-data">
        @csrf
