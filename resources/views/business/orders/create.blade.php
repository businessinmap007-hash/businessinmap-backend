...
@section('title', __('طلب جديد'))

@section('content')
<div class="a2-page-head">
    <div>
        <h1 class="a2-page-title">{{ __('طلب منيو جديد') }}</h1>
        <div class="a2-page-subtitle">{{ __('توصيل أو استلام. بعد الإنشاء تضيف الأصناف.') }}</div>
    </div>
...
<form method="POST" action="{{ route('business.orders.store') }}">
    ...
    <div class="a2-card a2-card--section">
        <div class="a2-form-grid">
            <div class="a2-form-group">
                <label class="a2-label" for="fulfillment_type">{{ __('نوع التنفيذ') }} <span class="a2-danger">*</span></label>
                <select class="a2-select js-order-type" id="fulfillment_type" name="fulfillment_type" required>
                    <option value="delivery" @selected(old('fulfillment_type','delivery')==='delivery')>{{ __('توصيل') }}</option>
                    <option value="pickup" @selected(old('fulfillment_type')==='pickup')>{{ __('استلام عند المرور') }}</option>
                </select>
            </div>

            <div class="a2-form-group js-order-delivery">
                <label class="a2-label" for="delivery_fee">{{ __('رسوم التوصيل') }}</label>
            ...
            <div class="a2-form-group a2-field-full js-order-delivery">
                <label class="a2-label" for="address">{{ __('عنوان التوصيل') }}</label>
                <input class="a2-input" id="address" name="address" value="{{ old('address') }}" placeholder="{{ __('العنوان بالتفصيل') }}">
            </div>

            <div class="a2-form-group a2-field-full">
                <label class="a2-label" for="notes">{{ __('ملاحظات') }}</label>
                <textarea class="a2-textarea" id="notes" name="notes" placeholder="{{ __('ملاحظات على الطلب') }}">{{ old('notes') }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="a2-page-actions" style="justify-content:flex-end;margin-top:16px;">
        <a href="{{ route('business.orders.index') }}" class="a2-btn a2-btn-ghost">{{ __('رجوع') }}</a>
        <button type="submit" class="a2-btn a2-btn-primary">{{ __('إنشاء') }}</button>
    </div>
</form>
