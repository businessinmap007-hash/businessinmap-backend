...
<head>
    ...
    <title>تأكيد التسليم — {{ config('app.name') }}</title>
    <style>
        :root {
            --bg: #f4f5f7; --card: #ffffff; --ink: #1c2430; --muted: #6b7686;
            --line: #e6e9ef; --brand: #2f6bff; --brand-ink: #ffffff; --ok: #157347; --danger: #b42318;
        ...
        * { box-sizing: border-box; }
        ...
        .big-ok { font-size: 40px; line-height: 1; }
...
<div class="wrap">
    ...
    {{-- Token gate: confirming must be authenticated as a party of the order. --}}
    <div id="gate" class="card hidden">
        <div class="sec-title">مطلوب تسجيل الدخول</div>
        <p class="muted" style="margin-top:0;font-size:13px;">لتأكيد استلام الطلب أدخل رمز الدخول الخاص بحسابك (API token).</p>
        <div class="field">
    ...
    <div id="intro" class="card hidden">
        <div class="sec-title">تأكيد استلام الطلب</div>
        <p class="muted" style="margin-top:0;">اضغط للتأكيد أن الطلب تم تسليمه/استلامه. لا يمكن التراجع بعد التأكيد.</p>
        <button id="confirm-btn" class="btn btn-ok">تأكيد الاستلام</button>
    </div>
    ...
    <div id="done" class="card center hidden">
        ...
        <div class="sec-title" style="margin-top:8px;">تم تأكيد الاستلام</div>
        <div class="muted">اكتمل الطلب رقم <span id="order-id">—</span>.</div>
    </div>
</div>
...
(function () {
    ...
    const CONFIRM_URL = '/api/v2/handover/' + encodeURIComponent(TOKEN) + '/confirm';
    ...
    function showGate() { hide('intro'); hide('loading'); hide('done'); show('gate'); }
    ...
    async function confirm() {
        ...
        let res;
        try { res = await fetch(CONFIRM_URL, { method: 'POST', headers: { 'Authorization': 'Bearer ' + token, 'Accept': 'application/json' } }); }
        ...
        if (res.status === 403) { hide('loading'); fail('لست طرفاً في هذا الطلب.'); return; }
        if (res.status === 404) { hide('loading'); fail('رمز التسليم غير صالح أو تم استخدامه.'); return; }
        if (res.status === 409) { hide('loading'); fail('لا يمكن تأكيد تسليم هذا الطلب.'); return; }
        if (!res.ok) { hide('loading'); fail('تعذّر تأكيد الاستلام.'); show('intro'); return; }
        let body = null; try { body = await res.json(); } catch (e) {}
        const id = body && body.data && body.data.order_id;
        $('order-id').textContent = id != null ? ('#' + id) : '';
    ...
    // Require an explicit tap to confirm (never auto-confirm on load).
    if (getToken()) show('intro'); else show('intro');
})();
</script>
</body>
</html>
