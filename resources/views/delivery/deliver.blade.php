...
<head>
    ...
    <title>تأكيد استلام الطلب — {{ config('app.name') }}</title>
    <style>
        :root {
...
<div class="wrap">
    ...
    {{-- Token gate: confirming receipt must be the customer who owns the order. --}}
    <div id="gate" class="card hidden">
        <div class="sec-title">مطلوب تسجيل الدخول</div>
        <p class="muted" style="margin-top:0;font-size:13px;">لتأكيد استلام طلبك أدخل رمز الدخول الخاص بحسابك (API token).</p>
        <div class="field">
    ...
    <div id="intro" class="card hidden">
        ...
        <p class="muted" style="margin-top:0;">اضغط للتأكيد أنك استلمت طلبك من الموصّل. لا يمكن التراجع بعد التأكيد.</p>
    ...
    <div id="done" class="card center hidden">
        ...
        <div class="muted">اكتمل توصيل الطلب رقم <span id="order-id">—</span> بنجاح.</div>
    </div>
</div>
...
(function () {
    ...
    const CONFIRM_URL = '/api/v2/delivery/deliver/' + encodeURIComponent(TOKEN) + '/confirm';
    ...
    async function confirm() {
        ...
        if (res.status === 403) { hide('loading'); fail('هذا الطلب ليس طلبك.'); return; }
        if (res.status === 404) { hide('loading'); fail('رمز التسليم غير صالح أو تم استخدامه.'); return; }
        if (res.status === 409) { hide('loading'); fail('لا يمكن تأكيد التسليم في هذه المرحلة.'); return; }
