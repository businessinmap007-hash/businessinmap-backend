...
<head>
    ...
    <title>تأكيد الاستلام من المطعم — {{ config('app.name') }}</title>
    <style>
        :root {
...
<div class="wrap">
    ...
    {{-- Token gate: confirming pickup must be the assigned driver. --}}
    <div id="gate" class="card hidden">
        <div class="sec-title">مطلوب تسجيل الدخول</div>
        <p class="muted" style="margin-top:0;font-size:13px;">لتأكيد استلام الطلب من المطعم أدخل رمز الدخول الخاص بحسابك (API token).</p>
        <div class="field">
    ...
    <div id="intro" class="card hidden">
        <div class="sec-title">تأكيد الاستلام من المطعم</div>
        <p class="muted" style="margin-top:0;">اضغط للتأكيد أنك استلمت الطلب من المطعم لبدء التوصيل.</p>
    ...
    <div id="done" class="card center hidden">
        ...
        <div class="muted">الطلب رقم <span id="order-id">—</span> قيد التوصيل الآن.</div>
    </div>
</div>
...
(function () {
    ...
    const CONFIRM_URL = '/api/v2/delivery/pickup/' + encodeURIComponent(TOKEN) + '/confirm';
    ...
    async function confirm() {
        ...
        if (res.status === 403) { hide('loading'); fail('هذا الطلب غير مُسنَد إليك.'); return; }
        if (res.status === 404) { hide('loading'); fail('رمز الاستلام غير صالح أو تم استخدامه.'); return; }
        if (res.status === 409) { hide('loading'); fail('لا يمكن تأكيد الاستلام في هذه المرحلة.'); return; }
